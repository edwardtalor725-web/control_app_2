...
def operations():
    if request.method == 'POST':
        ...
        operation = Operation(
            ...
            planned_date=(datetime.strptime(request.form.get('planned_date'), '%Y-%m-%d').date() if request.form.get('planned_date') else None),
...
def edit_operation(op_id):
    ...
    if request.method == 'POST':
        ...
        op.date = datetime.strptime(request.form['date'], '%Y-%m-%d').date()
        db.session.commit()
...
def reports_export():
    ...
    if fmt == 'csv' or fmt == 'excel':
        ...
        for op in operations:
            writer.writerow([op.date.isoformat(), op.type, op.amount, op.category, op.description or ''])
        output.seek(0)
