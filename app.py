...
def operations():
    if request.method == 'POST':
        ...
        operation = Operation(
            ...
            is_planned='is_planned' in request.form,
            planned_date=datetime.strptime(request.form['planned_date'], '%Y-%m-%d').date() if request.form.get('planned_date') else None,
            status='planned' if 'is_planned' in request.form else 'completed',
            user_id=current_user.id,
            counterparty_id=request.form['counterparty_id'] if request.form.get('counterparty_id') else None
        )
    ...
    if request.args.get('type'):
        operations_query = operations_query.filter_by(type=request.args['type'])
    if request.args.get('start_date'):
        start_date = datetime.strptime(request.args['start_date'], '%Y-%m-%d').date()
        operations_query = operations_query.filter(Operation.date >= start_date)
...
@login_required
def calendar():
    planned_operations = Operation.query.filter(
    ...
    for op in planned_operations:
        events.append({
            'id': op.id,
            'title': f"{'Доход' if op.type == 'income' else 'Расход'}: {op.description[:30]} - {op.amount} руб.",
...
def init_database():
    with app.app_context():
        ...
        if not User.query.first():
            ...
            accounts = [
                Account(name='Основной счет', balance=100000, min_balance=50000),
                Account(name='Резервный счет', balance=50000, min_balance=10000)
            ]
            for acc in accounts:
                db.session.add(acc)
            ...
            counterparties = [
                ...
                Counterparty(name='ООО "Клиент Плюс"', type='client', contact_info='+7 495 123-45-67')
            ]
